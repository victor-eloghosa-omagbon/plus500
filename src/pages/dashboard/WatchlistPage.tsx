            ...
            <div key={item.symbol}>
              ...
              <div className="hidden sm:grid grid-cols-[1fr_auto_auto_auto_auto_auto] gap-4 px-4 py-3 border-b border-border last:border-0 hover:bg-muted/50 transition-colors items-center">
                ...
                <p className="text-sm font-mono text-foreground text-right">
                  {item.bid < 10 ? item.bid.toFixed(4) : item.bid.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </p>
                <p className="text-sm font-mono text-foreground text-right">
                  {item.ask < 10 ? item.ask.toFixed(4) : item.ask.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </p>
                <p className={cn("text-xs font-semibold text-right", item.change >= 0 ? "text-profit-green" : "text-loss-red")}>
              ...
              <div className="sm:hidden border-b border-border last:border-0 px-4 py-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={cn("text-xs font-semibold", item.change >= 0 ? "text-profit-green" : "text-loss-red")}>
                      {item.change >= 0 ? "+" : ""}{item.change.toFixed(2)}%
                    </span>
                    <button onClick={() => removeFromWatchlist(item.symbol)} className="p-1 rounded hover:bg-muted transition-colors">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-4 text-xs text-muted-foreground">
                    <span>Bid: <span className="font-mono text-foreground">{item.bid < 10 ? item.bid.toFixed(4) : item.bid.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></span>
                    <span>Ask: <span className="font-mono text-foreground">{item.ask < 10 ? item.ask.toFixed(4) : item.ask.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></span>
                  </div>
                  <Button
                    size="sm"
